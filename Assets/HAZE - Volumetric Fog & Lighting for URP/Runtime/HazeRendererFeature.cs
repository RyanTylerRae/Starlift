...
#region Density Volume Data
        ...
        private class FroxelFogRenderPass : ScriptableRenderPass
        {
            ...
            public FroxelFogRenderPass(FroxelFogPassSettings settings)
            {
                ...
                var bufferDescriptor = new RenderTextureDescriptor
                {
                ...
                RenderingUtils.ReAllocateHandleIfNeeded(ref _scatterBufferHandle, in bufferDescriptor,
            ...
            public override void OnCameraSetup(CommandBuffer cmd, ref RenderingData renderingData)
            {
                ...
                RenderingUtils.ReAllocateHandleIfNeeded(ref _colorDensityBufferHandle, in bufferDescriptor,
                    FilterMode.Trilinear, TextureWrapMode.Clamp);
                RenderingUtils.ReAllocateHandleIfNeeded(ref _colorDensityHistoryBufferHandle, in bufferDescriptor,
                    FilterMode.Trilinear, TextureWrapMode.Clamp);
                RenderingUtils.ReAllocateHandleIfNeeded(ref _scatterBufferHandle, in bufferDescriptor,
                    FilterMode.Trilinear, TextureWrapMode.Clamp);
            }
            ...
            public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
            {
                ...
                _prevViewProjectionMatrixDictionary.TryAdd(cameraData.camera, Matrix4x4.identity);
                ...
                cmd.SetComputeMatrixParam(froxelFogComputeShader, PrevViewProjectionMatrix, _prevViewProjectionMatrixDictionary[cameraData.camera]);
                ...
                if (temporalReprojection)
                {
                    ...
                    _prevViewProjectionMatrixDictionary[cameraData.camera] = cameraData.camera.orthographic ? viewMatrix : viewProjectionMatrix;
                }
            ...
            public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
            {
                ...
                TextureHandle colorDensityBuffer;
                TextureHandle colorDensityHistoryBuffer;
                ...
                {
                    cameraData.historyManager.RequestAccess<HazeBufferHistoryItem>();
                    var history = cameraData.historyManager.GetHistoryForWrite<HazeBufferHistoryItem>();

                    history?.Update(bufferDescriptor);

                    colorDensityBuffer = renderGraph.ImportTexture(history?.CurrentTexture, _renderTargetInfo);
                    colorDensityHistoryBuffer = renderGraph.ImportTexture(history?.PreviousTexture, _renderTargetInfo);
                }
                else
                {
                    RenderingUtils.ReAllocateHandleIfNeeded(ref _colorDensityBufferHandle, in bufferDescriptor,FilterMode.Trilinear, TextureWrapMode.Clamp);
                    RenderingUtils.ReAllocateHandleIfNeeded(ref _colorDensityHistoryBufferHandle, in bufferDescriptor, FilterMode.Trilinear, TextureWrapMode.Clamp);
                    
                    colorDensityBuffer = renderGraph.ImportTexture(_colorDensityBufferHandle, _renderTargetInfo);
                    colorDensityHistoryBuffer = renderGraph.ImportTexture(_colorDensityHistoryBufferHandle, _renderTargetInfo);
                }
                ...
                using (var builder = renderGraph.AddComputePass("Color density accumulation", out DensityGatherPassData passData, profilingSampler))
                {
                    ...
                    passData.PrevViewProjectionMatrix = _prevViewProjectionMatrixDictionary[cameraData.camera];
                    ...
                    passData.TemporalReprojection = (Application.isPlaying && cameraData.cameraType == CameraType.Game) || !Application.isPlaying;
